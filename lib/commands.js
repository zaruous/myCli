...
import { confirmPrompt, resolveToggle, printTrace } from './ui.js';
import { getTimestamp, getSafePath } from './utils.js';
import { getBaseDir, setBaseDir, planModeState, displayState } from './state.js';
...
import { getLoadedHooks } from './hooks.js';
import { queryHookEvents, countHookEvents, clearHookEvents, isLoggerReady } from './hook-logger.js';
...
export function registerCommands(program, cliState, refs) {
  ...
  program
    ...
    .action(async () => {
      const builtinCmds = [
        ...
        { cmd: '/model [provider]',    desc: '모델 전환 (gemini | gpt | ollama | vllm)' },
        ...
        { cmd: '/thinking [on|off]',   desc: '도구 호출 입력·결과(중간 과정) 표시 접기/펴기 (생략 시 토글)' },
        { cmd: '/skills [on|off]',     desc: '스킬 로드 시 주입 프롬프트 전문 표시 접기/펴기 (생략 시 토글)' },
        ...
        { cmd: '/hooks',               desc: '로드된 훅 설정 목록 출력' },
        { cmd: '/hook-log [--event E] [--tool T] [--limit N] [--clear]', desc: '훅 이벤트 SQLite 로그 조회' },
  ...
  program
    .command('/clear')
    .description('대화 기록 및 콘솔 화면을 모두 지웁니다.')
    .action(async () => {
      await memory.clear();
      process.stdout.write('\x1b[2J\x1b[H'); // 화면 지우기 + 커서 맨 위로
      console.log(chalk.yellow('✅ 채팅 기록과 화면이 초기화되었습니다.'));
      askQuestion();
    });
  ...
  program
    ...
    .description('AI 모델을 전환합니다. provider: gemini | gpt | ollama | vllm')
    .action(async (provider) => {
      const valid = ['gemini', 'gpt', 'ollama', 'vllm'];
      if (!provider) {
        console.log(chalk.bold('\n사용 가능한 모델:'));
      ...
      if (!valid.includes(provider)) {
        console.log(chalk.red(`❌ 알 수 없는 provider: ${provider}. (gemini | gpt | ollama | vllm)`));
  ...
  program
    ...
    .action(async () => {
      ...
      console.log(`  ${'중간 과정'.padEnd(14)} ${displayState.thinking ? chalk.cyan('펼침 (/thinking)') : chalk.gray('접힘 (/thinking)')}`);
      console.log(`  ${'스킬 프롬프트'.padEnd(14)} ${displayState.skills ? chalk.cyan('펼침 (/skills)') : chalk.gray('접힘 (/skills)')}`);
  ...
  program
    ...
    .action(async () => {
      ...
      try {
        const model = await getModel(cliState.currentProvider);
  ...
  program
    ...
    .action(async () => {
      ...
      const confirmed = await confirmPrompt(`마지막 파일 수정을 되돌리시겠습니까? (${getUndoCount()}건 남음)`);
  ...
  program
    .command('/hooks')
    .description('로드된 훅 설정 목록을 출력합니다.')
    .action(async () => {
      const hooks = getLoadedHooks();
      const events = Object.keys(hooks);

      console.log(chalk.bold('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
      console.log(chalk.bold.cyan('  로드된 훅 설정'));
      console.log(chalk.bold('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));

      if (events.length === 0) {
        console.log(chalk.gray('  등록된 훅이 없습니다.'));
        console.log(chalk.gray('  ~/.mycli/hooks.json 또는 .mycli/hooks.json 에 훅을 추가하세요.'));
      } else {
        for (const event of events) {
          const entries = hooks[event];
          console.log(`\n  ${chalk.cyan.bold(event)} ${chalk.gray(`(${entries.length}개 항목)`)}`);
          for (const entry of entries) {
            console.log(`    matcher: ${chalk.yellow(entry.matcher ?? '.*')}`);
            for (const h of (entry.hooks ?? [])) {
              console.log(`      ${chalk.green('→')} ${h.command}`);
            }
          }
        }
      }

      const logReady = isLoggerReady();
      const logStatus = process.env.MYCLI_HOOK_LOG === 'false'
        ? chalk.gray('비활성 (MYCLI_HOOK_LOG=false)')
        : logReady
          ? chalk.green('활성 (SQLite 로깅 중)')
          : chalk.yellow('초기화 실패');
      console.log(`\n  ${'SQLite 로거'.padEnd(12)} ${logStatus}`);
      console.log(chalk.bold('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n'));

      await saveHistory('/hooks');
      askQuestion();
    });
  ...
  program
    .command('/hook-log')
    .description('훅 이벤트 SQLite 로그를 조회합니다.')
    .option('--event <name>',   '특정 이벤트만 필터 (PreToolCall 등)')
    .option('--tool <name>',    '특정 도구만 필터')
    .option('--limit <n>',      '출력 건수 (기본 20)', '20')
    .option('--clear',          '전체 로그 삭제')
    .action(async (opts) => {
      if (!isLoggerReady()) {
        const reason = process.env.MYCLI_HOOK_LOG === 'false'
          ? 'MYCLI_HOOK_LOG=false 로 비활성화되어 있습니다.'
          : 'SQLite 로거 초기화에 실패했습니다.';
        console.log(chalk.yellow(`훅 로그를 사용할 수 없습니다: ${reason}`));
        await saveHistory('/hook-log');
        askQuestion();
        return;
      }

      if (opts.clear) {
        const confirmed = await confirmPrompt('모든 훅 로그를 삭제하시겠습니까?');
        if (confirmed) {
          const deleted = clearHookEvents();
          console.log(chalk.green(`✅ ${deleted}건 삭제되었습니다.`));
        } else {
          console.log(chalk.gray('취소했습니다.'));
        }
        await saveHistory('/hook-log --clear');
        askQuestion();
        return;
      }

      const limit  = parseInt(opts.limit, 10) || 20;
      const filter = { event: opts.event, tool: opts.tool, limit };
      const rows   = queryHookEvents(filter);
      const total  = countHookEvents({ event: opts.event, tool: opts.tool });

      console.log(chalk.bold('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
      console.log(chalk.bold.cyan('  훅 이벤트 로그') + chalk.gray(`  (전체 ${total}건)`));
      console.log(chalk.bold('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));

      if (rows.length === 0) {
        console.log(chalk.gray('  기록된 이벤트가 없습니다.'));
      } else {
        const EVENT_COLOR = {
          PreUserPromptSubmit: chalk.blue,
          PreToolCall:         chalk.yellow,
          PostToolCall:        chalk.green,
          Stop:                chalk.magenta,
        };

        for (const row of rows) {
          const color = EVENT_COLOR[row.event] ?? chalk.white;
          const ts    = new Date(row.created_at).toLocaleString('ko-KR');
          console.log(`\n  ${chalk.gray('#' + row.id)}  ${color(row.event.padEnd(22))}  ${chalk.gray(ts)}`);
          if (row.tool_name)  console.log(`    도구      ${chalk.cyan(row.tool_name)}`);
          if (row.user_input) console.log(`    입력      ${chalk.white(row.user_input.slice(0, 80))}${row.user_input.length > 80 ? '…' : ''}`);
          if (row.tool_input) {
            try {
              const parsed = JSON.parse(row.tool_input);
              const preview = JSON.stringify(parsed).slice(0, 100);
              console.log(`    tool_input ${chalk.gray(preview)}${preview.length >= 100 ? '…' : ''}`);
            } catch {
              console.log(`    tool_input ${chalk.gray(row.tool_input.slice(0, 100))}`);
            }
          }
          if (row.tool_output) console.log(`    결과      ${chalk.gray(row.tool_output.slice(0, 80))}${row.tool_output.length > 80 ? '…' : ''}`);
          if (row.ai_output)   console.log(`    AI 응답   ${chalk.gray(row.ai_output.slice(0, 80))}${row.ai_output.length > 80 ? '…' : ''}`);
          console.log(`    provider  ${chalk.gray(row.provider ?? '-')}   base_dir  ${chalk.gray(row.base_dir ?? '-')}`);
        }

        if (rows.length < total) {
          console.log(chalk.gray(`\n  ... ${total - rows.length}건 더 있습니다. --limit 으로 더 불러오세요.`));
        }
      }
  ...
  // ── /thinking · /skills (표시 접기/펴기) ─────────────────
  const displayToggles = [
    { cmd: 'thinking', key: 'thinking', label: '중간 과정(도구 호출 입력·결과)' },
    { cmd: 'skills',   key: 'skills',   label: '스킬 주입 프롬프트 전문' },
  ];
  for (const { cmd, key, label } of displayToggles) {
    program
      .command(`/${cmd} [state]`)
      .description(`${label} 표시를 접거나 펼칩니다. state: on | off (생략 시 토글)`)
      .action(async (state) => {
        const next = resolveToggle(state, displayState[key]);
        if (next === null) {
          console.log(chalk.red(`❌ 잘못된 값: ${state}. (on | off)`));
        } else {
          displayState[key] = next;
          console.log(next
            ? chalk.green(`▼ ${label} 표시: 펼침 (on)`)
            : chalk.gray(`▶ ${label} 표시: 접힘 (off)`));
        }
        await saveHistory(`/${cmd}${state ? ' ' + state : ''}`);
        askQuestion();
      });
  }
  ...
  for (const skill of skills) {
    program
      ...
      .action(async (args) => {
        ...
        if (displayState.skills) printTrace(`[스킬] '/${skill.name}' 프롬프트:`, finalPrompt, Infinity);
