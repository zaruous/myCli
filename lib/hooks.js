...
export async function loadHooks() {
  ...
  // 프로젝트 로컬 훅은 BASE_DIR 과 cwd 양쪽에서 탐색
  const localPaths = resolveProjectPaths('.mycli', 'hooks.json');
  ...
  // 전역 → 로컬 순으로 이벤트별 merge (뒤에 추가)
  _hooks = { ...(await readHooks(globalPath)) };
  for (const localPath of localPaths) {
    const localHooks = await readHooks(localPath);
