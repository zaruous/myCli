    ...
    func: async ({ filePath, content }) => {
      ...
      try {
        ...
        const { confirmed } = await inquirer.prompt([{
          ...
          message: isNew ? '이 파일을 생성하시겠습니까?' : '이 변경사항을 적용하시겠습니까?',
          default: true,
        }]);
  ...
  new DynamicStructuredTool({
    ...
    func: async ({ plan }) => {
      ...
      const { decision } = await inquirer.prompt([{
        type: 'expand', name: 'decision',
        message: '이 계획을 승인하시겠습니까?',
        default: 'y',
        choices: [
          { key: 'y', name: '승인 - 이 계획으로 구현을 시작합니다', value: 'approve'  },
          { key: 'n', name: '거절 - 계획을 수정합니다',              value: 'reject'   },
          { key: 'e', name: '수정 제안 - 피드백을 남기고 재계획합니다', value: 'feedback' },
        ],
      }]);
      ...
      if (decision === 'feedback') {
        const { feedback } = await inquirer.prompt([{ type: 'input', name: 'feedback', message: '수정 방향이나 피드백을 입력하세요:' }]);
