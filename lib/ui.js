...
// ─────────────────────────────────────────────────────────────
// 접기/펴기 표시 유틸 (/thinking, /skills)
// ─────────────────────────────────────────────────────────────
export const TRACE_MAX_CHARS = 500;

/**
 * on | off 인자를 해석한다. 생략하면 현재 값을 반전(토글)한다.
 * @param {string|undefined} arg
 * @param {boolean} current
 * @returns {boolean|null} 잘못된 값이면 null
 */
export function resolveToggle(arg, current) {
  if (arg === undefined || arg === null || arg === '') return !current;
...
/**
 * 도구 입력/결과 등 임의 값을 트레이스용 문자열로 바꾼다.
 * LangChain 메시지(ToolMessage 등) → content, 객체 → JSON, 길면 절단.
 * @param {*} value
 * @param {number} max 최대 글자 수 (Infinity 면 절단 없음)
 */
export function toTraceText(value, max = TRACE_MAX_CHARS) {
  let text;
  if (value === undefined || value === null) text = '';
  else if (typeof value === 'string') text = value;
  else if (typeof value === 'object' && typeof value._getType === 'function') {
...
/** 트레이스 블록 출력: 회색 제목 한 줄 + 들여쓴 본문 */
export function printTrace(title, value, max = TRACE_MAX_CHARS) {
  const body = toTraceText(value, max);
  process.stdout.write(chalk.gray(`  ${title}\n`));
  if (body) process.stdout.write(chalk.dim(body.split('\n').map(l => '    ' + l).join('\n')) + '\n');
}
    ...
    const createRl = (initialValue = '') => {
      ...
      if (filePicker) {
        atHandler = async (str) => {
          ...
          // 단어 중간의 '@' (이메일, express@4 등) 는 파일 피커를 열지 않는다.
          // 줄 처음이나 공백 뒤의 '@' 만 첨부 의도로 본다.
          const beforeChar = (currentLine.endsWith('@') ? currentLine.slice(0, -1) : currentLine).slice(-1);
          if (beforeChar && !/\s/.test(beforeChar)) return;
