...
import os from 'os';
import { selectKeyPrompt } from './ui.js';
...
export async function paginate(text, pageSize = 20) {
  ...
  while (offset < lines.length) {
    ...
    if (offset < lines.length) {
      ...
      const action = await selectKeyPrompt(
        `  ── ${offset}/${lines.length}줄 (남은 ${remaining}줄) ──`,
        [
          { key: 'n', label: '다음 페이지', value: 'next' },
          { key: 'a', label: '전체 출력',   value: 'all'  },
          { key: 'q', label: '중단',        value: 'quit' },
        ], 'n'
      );
